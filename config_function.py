...
import pandas as pd
from src.utils.pytorch.ts_dataset_ex1 import TimeSeriesDataset
...
def start(Y_df, Y_df2, windows_size, stack_types, n_blocks, string):
    X_df = Y_df.iloc[:]
    X_df2 = Y_df2.iloc[:]
    # X_df2 = Y_df2.iloc[:, [0, 1, 3]]
    train_mask = np.ones(len(Y_df))
    train_mask[-int(0.1 * len(Y_df)):] = 0  # Last week of data (168 hours)
    val_mask = np.zeros(len(Y_df))
    val_mask2 = np.zeros(len(Y_df2))
    ...
    ts_dataset = TimeSeriesDataset(Y_df=Y_df, X_df=X_df,
                                   ts_train_mask=train_mask)
    ts_dataset_val = TimeSeriesDataset(Y_df=Y_df2, X_df=X_df2,
                                       ts_train_mask=val_mask2)
    ...
    model = Nbeats(input_size_multiplier=windows_size,
                   ...
                   n_blocks=n_blocks,
                   n_layers=[3, 3, 3, 3, 3],
                   n_hidden=[[512, 512, 512], [512, 512, 512], [512, 512, 512], [512, 512, 512], [512, 512, 512]],
                   ...
                   random_seed=1,
                   string = string)
    # lr_decay_steps = n_iterations // n_lr_decay_steps
    # In[5]:
    ...
    data = block_forecasts.reshape((len(Y_df2))-1, sum(n_blocks)+1, 1).mean(axis=2)
