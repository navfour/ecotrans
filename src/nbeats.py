...
from src.nbeats_model import NBeats, NBeatsBlock, IdentityBasis, TrendBasis, SeasonalityBasis
from src.nbeats_model import ExogenousBasisInterpretable, ExogenousBasisWavenet, ExogenousBasisTCN, \
    ExogenousBasisTransformer
...
from src.utils.numpyse.metrics import mae, pinball_loss
...
class Nbeats(object):
    ...
    def __init__(self,input_size_multiplier,
                 output_size,
                 shared_weights,
                 activation,
                 initialization,
        ...
        self.t_cols = t_cols
        self.train_mask_scale = train_mask_scale
        self.string = string
        if device is None:
    ...
    def create_stack(self):
        ...
        x_t_n_inputs = self.exogenous_n_channels
        # ------------------------ Model Definition ---------------------  ---#
        block_list = []
        ...
        for i in range(len(self.stack_types)):
            for block_id in range(self.n_blocks[i]):
                ...
                if self.shared_weights and block_id > 0:
                    ...
                    elif self.stack_types[i] == 'exogenous_transformer':
                        nbeats_block = NBeatsBlock(x_t_n_inputs=x_t_n_inputs,
                                                   x_s_n_inputs=self.n_x_s,
                                                   x_s_n_hidden=self.x_s_n_hidden,
                                                   theta_n_dim=2 * (self.exogenous_n_channels),
                                                   basis=ExogenousBasisTransformer(c_in=self.n_x_t, seq_len=int(
                                                       120 * self.train_mask_scale),
                                                                                   c_out=self.exogenous_n_channels,
                                                                                   train_mask_scale=self.train_mask_scale,
                                                                                   dropout=0),
                                                   n_layers=self.n_layers[i],
                                                   theta_n_hidden=self.n_hidden[i],
                                                   include_var_dict=self.include_var_dict,
    ...
    def fit(self, train_ts_loader, val_ts_loader=None, n_iterations=None, verbose=True, eval_steps=1):
        ...
        if not self._is_instantiated:
            ...
            # 初始化blocklist
            self.model = NBeats(t.nn.ModuleList(block_list), self.exogenous_n_channels, len(self.t_cols)-2, self.string).to(self.device)
        ...
        while (iteration < n_iterations) and (not break_flag):
            epoch += 1
            # aaaaa=train_ts_loader.ts_windows.numpy()
            # pprint(train_ts_loader.ts_windows)
            # pprint(train_ts_loader.ts_windows.numpy())
        ...
        if n_iterations > 0:
            # This is batch loss!
            self.final_insample_loss = np.float(
                training_loss.cpu().data.numpy()) if not break_flag else best_insample_loss
        ...
        with t.no_grad():
            for batch in iter(ts_loader):
                ...
                forecast, block_forecast = self.model(insample_y=insample_y, insample_x_t=insample_x,
                                                      ...
                                                      x_s=s_matrix,
                                                      return_decomposition=True)  # always return decomposition
