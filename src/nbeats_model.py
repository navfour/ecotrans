...
class NBeats(nn.Module):
    ...
    def forward(self, insample_y: t.Tensor, insample_x_t: t.Tensor, insample_mask: t.Tensor,
        ...
        forcast_informer = forcast_informer.unsqueeze(dim=1)
        forecast = forecast + forcast_informer
        block_forecasts.append(forcast_informer)
...
class ExogenousBasisWavenet(nn.Module):
    # out_features是windows_size，in_features是有几个外部数据
...
class ExogenousBasisTransformer(nn.Module):
    def __init__(self, c_in, seq_len, c_out, train_mask_scale, dropout=0):
        super().__init__()
        self.transformer = Transformer(c_in=c_in, seq_len=seq_len, c_out=c_out, train_mask_scale=train_mask_scale,
                                       dropout=dropout)
