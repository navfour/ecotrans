...
class TimeSeriesDataset(Dataset):
    def __init__(self,
        ...
        self.max_len = max([len(ts['y']) for ts in ts_data])  # 119
        self.n_channels = len(self.t_cols)  # 3  # y, X_cols, insample_mask and outsample_mask
    ...
    def _df_to_lists(self, Y_df, S_df, X_df):
        ...
        if X_df is not None:
            X_cols = [col for col in X_df.columns if col not in ['unique_id', 'ds', 'y']]
        else:
            X_cols = []
    ...
    def get_filtered_ts_tensor(self, offset, output_size, window_sampling_limit, ts_idxs=None):
        ...
        print(np.sum(np.isnan(filtered_ts_tensor)))
